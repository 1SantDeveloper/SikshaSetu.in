 ...
 */
@MultipartConfig
public class AddQuestionLink extends HttpServlet {
...
String category=request.getParameter("category");
            String link = request.getParameter("filelink");
            
            HttpSession usr = request.getSession();
            RegisterUser ru = (RegisterUser) usr.getAttribute("cuser");
            int userid = ru.getUserId();
            out.println(subject + " " + university + course + semester + year + link + userid);
            QuestionEntity QE = new QuestionEntity(subject, university,category, course, semester, year, userid,link);
            try {
                QuestionDao QD = new QuestionDao(ConnectionProvider.getConnect());
                Boolean r = QD.saveQuestion(QE,5);
                if (r) {
//                    Boolean f = ImageHelper.imageSave(filepart.getInputStream(), filelocation);
                    ...
                    response.sendRedirect("AddQuestionlink.jsp");
                } else {
